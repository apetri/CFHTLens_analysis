...
found modest ($10-20\%$) improvements over the 2PCF. \textbf{\color{blue} These results rely on the third order statistics systematic tests performed by Simon et al. \citep{CFHTSimon}}.  
...
$\pmb{\beta}(\chi_s;\pmb{\theta})$. \textbf{\color{blue} Since light rays travel undeflected from the observer to the first lens plane, the derivative initial condition in the Cauchy problem (\ref{raytrajectory}) reads $\dot{\pmb{\beta}}(0;\pmb{\theta})=0$. We indicate the derivative of $\pmb{\beta}$ with respect to $\chi$ as $\dot{\pmb{\beta}}$.} 
%
We use our proprietary implementation Inspector Gadget~(see, e.g., \cite{KHM2010}) to solve
for the light ray trajectories based \textbf{\color{blue} on a discretized version of equation (\ref{raytrajectory}) that is based on the multi--lens--plane algorithm (see \citep{RayTracingHartlap} for example).
...
first and third Pauli matrices. \textbf{\color{blue} We perform this procedure for each of the $R$ realizations of the lens planes and we obtain $R$ pseudo--independent realizations of the $\pmb{\gamma}$ weak lensing field.} We use these different random realizations to estimate the means \textbf{\color{blue} (from the \texttt{CFHTemu1} simulations)} and covariance matrices \textbf{\color{blue} (from the \texttt{CFHTcov} simulations)} of our descriptors. \textbf{\color{blue} Since the random box rotations and translations that make up the \texttt{CFHTcov} simulations are based on 50 independent $N$--body runs, we believe the covariance matrices measured from this set to be more accurate than the ones measured from the \texttt{CFHTemu1} set.} 
...
contains the lensing shear by large scale structure \textbf{\color{blue} (LSS)}, but the random
rotation makes this contribution \textbf{\color{blue} at least} second order in $\kappa$ \textbf{\color{blue} by destroying the shape spatial correlations induced by lensing from LSS}. Consistent with the weak lensing approximation, the lensing signal from the
...
finite differences. \textbf{\color{blue} In this notation $\Theta(x)$ is the Heaviside function}
%
\begin{equation}
\Theta(x) = 
\begin{cases}
...
\end{equation}
%
\textbf{\color{blue} and $\delta_D(x)$ is the Dirac delta function.}
...
measured from an average over the $R=1000$ realizations \textbf{\color{blue} in the \texttt{CFHTemu1} ensemble}  
\begin{equation}
...
\end{equation}
is measured from the \textbf{\color{blue} $R=1000$} realizations \textbf{\color{blue} in the \texttt{CFHTcov} ensemble}.
...
spectrum emulator has a \textbf{\color{blue} relative error} smaller than 20\% for the lower
multipoles ($l<500$), and comparable to 1\% for the higher multipoles. The MF
emulator has a \textbf{\color{blue} relative error} $\lesssim$10\% for the first 30 bins (which
...
scheme~\footnote{Following ref.~\citep{coyote2}, we have experimented with interpolations using a Gaussian Process, but did not find a significant improvement in \textbf{\color{blue} relative error} over the much simpler polynomial scheme; this is in agreement with earlier results (see for example \citep{KnoxGP})}.
...
$\Sigma_8(\alpha)\equiv \sigma_8(\Omega_m/0.27)^\alpha$. \textbf{\color{blue} In the next section} we give a justification on why we fix $\alpha$ to a value of $0.55$. 
...
$\Sigma_8$ in Figure \ref{likelihoodSi8cross}. \textbf{\color{blue} We also provide a tabulated version of the $\Sigma_8$ constraints ($1\sigma$) in Table \ref{Sigma8Table}}. We discuss these findings in the next section.
...
compute confidence levels. \textbf{\color{blue} Since the Fisher matrix formalism is equivalent to a linear approximation of our emulator (in which all cosmological parameter dependencies are assumed to be linear)}, we thus attribute this bias to the \textbf{\color{blue} oversimplifying} assumption of linear cosmology-parameter dependence of the descriptors.  
...
combined, $w$ remains essentially unconstrained. \textbf{\color{blue} \citep{CFHTHeymansTomo} found that even weak lensing tomography alone is unable to constrain $w$ sensibly}. 
